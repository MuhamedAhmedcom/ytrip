  ...
  class CSF_Field_typography extends CSF_Fields {
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        // Use local WebFontLoader (bundled with YTrip)
        $webfontloader_url = YTRIP_URL . 'assets/vendor/webfontloader/webfontloader.min.js';
        
        // Fallback to check if local file exists, otherwise use a safe fallback
        if ( ! file_exists( YTRIP_PATH . 'assets/vendor/webfontloader/webfontloader.min.js' ) ) {
          // Use inline WebFontLoader as ultimate fallback
          $webfontloader_url = '';
        }
        
        if ( ! empty( $webfontloader_url ) ) {
          wp_enqueue_script( 'csf-webfontloader', esc_url( $webfontloader_url ), array( 'csf' ), '1.6.28', true );
        }
