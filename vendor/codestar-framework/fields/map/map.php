...
if ( ! class_exists( 'CSF_Field_map' ) ) {
  class CSF_Field_map extends CSF_Fields {
    ...
    public $cdn_url = 'https://cdn.jsdelivr.xyz/npm/leaflet@';
    public $local_url = '';

    public function __construct( $field, $value = '', $unique = '', $where = '', $parent = '' ) {
      parent::__construct( $field, $value, $unique, $where, $parent );
      
      $this->local_url = YTRIP_URL . 'assets/vendor/leaflet/';
    }
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-leaflet' ) ) {
        wp_enqueue_script( 'csf-leaflet', esc_url( $this->local_url . 'leaflet.js' ), array( 'csf' ), $this->version, true );
      }

      if ( ! wp_style_is( 'csf-leaflet' ) ) {
        wp_enqueue_style( 'csf-leaflet', esc_url( $this->local_url . 'leaflet.css' ), array(), $this->version );
      }
